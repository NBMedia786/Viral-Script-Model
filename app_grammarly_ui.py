...
import os, re, glob, json, tempfile, difflib, uuid, datetime, shutil, time, random
from pathlib import Path
from typing import Dict, Any, List, Tuple, Optional
...
# ---------- HARDENED S3 CLIENT (drop-in replacement) ----------
@st.cache_resource(show_spinner=False)
def _s3_client():
    if not _s3_enabled():
        return None
    ...
    endpoint = (_RP_ENDPOINT or "").strip().rstrip("/")
    if endpoint and not endpoint.startswith(("http://", "https://")):
        endpoint = "https://" + endpoint  # be explicit; most gateways want TLS

    cfg = Config(
        signature_version="s3v4",
        s3={"addressing_style": "path" if _FORCE_PATH else "auto"},
        retries={"max_attempts": 8, "mode": "adaptive"},   # stronger retries
        connect_timeout=5,
        read_timeout=30,
    )
    return boto3.client(
        ...
        region_name=_RP_REGION or "us-east-1",
        use_ssl=_USE_SSL,
        verify=_VERIFY_SSL,
        config=cfg,
        **session_kwargs,
...
# ---------- CACHED HEALTH CHECK (new) ----------
@st.cache_data(ttl=60, show_spinner=False)
def _s3_ping(bucket: str) -> tuple[bool, str]:
    """Lightweight health check with retries (cached for 60s)."""
    s3 = _s3_client()
    if s3 is None:
        return (False, "client_unavailable")
    kwargs = {"Bucket": bucket, "Prefix": "", "MaxKeys": 1, "Delimiter": "/"}
    last = ""
    for attempt in range(5):
        try:
            ...
            last = str(e)
            time.sleep(0.3 + random.random() * (0.7 + attempt * 0.2))
    return (False, last or "unknown_error")
...
def _assert_s3_ready():
    ...
    ok, msg = _s3_ping(_RP_BUCKET)
    if not ok:
        st.error("Cannot reach Runpod S3 (transient gateway error or bad config).")
        st.caption("Health check is cached for 60s to avoid hammering the gateway.")
        st.code(str(msg))
        st.stop()
...
# ---------- RESILIENT list_prefix (drop-in replacement) ----------
def list_prefix(prefix: str, max_items: int = 500) -> List[str]:
    if not _s3_enabled():
        return []
    s3 = _s3_client()
    out: List[str] = []
    ...
    def _page(token_):
        kwargs = {
            ...
            "Prefix": base,
            "Delimiter": "/",
            "MaxKeys": 200,  # keep pages small
        }
        if token_:
            kwargs["ContinuationToken"] = token_
    ...
    while True:
        try:
            ...
            if not token:
                break
            tries = 0  # reset after a good page
        except (EndpointConnectionError, ClientError, NoCredentialsError):
            tries += 1
            if tries <= 5:
                time.sleep(0.3 * tries + random.random() * 0.4)
                continue
            break  # give back what we have
        except Exception:
            break
    return out
...
# CACHED manifest read (drop-in replacement)
