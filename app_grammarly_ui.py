...
# - Recents: browse history (title + timestamp + overall), open any run exactly as generated
# - FIX: history loader normalizes created_at (epoch/str/missing) => consistent ISO; safe sort
# - NEW: history saved to a WRITABLE data dir on Streamlit Cloud; Recents merge data+repo; source chip
...
# - Recents: browse history (title + timestamp + overall), open any run exactly as generated
# - FIX: history loader normalizes created_at (epoch/str/missing) => consistent ISO; safe sort
# - NEW: loads .env locally; Streamlit Cloud uses st.secrets via review_engine_multi/utils1
# - NEW: safe imports for utils1 and review_engine_multi with readable traceback
...
# ---------- Folders (read-only repo vs writable data) ----------
APP_ROOT = Path(__file__).resolve().parent  # e.g., /mount/src/viral-script-model (read-only on Cloud)
REPO_PROMPTS_DIR = APP_ROOT / "prompts"
REPO_SCRIPTS_DIR = APP_ROOT / "scripts"
REPO_HISTORY_DIR = APP_ROOT / "outputs" / "_history"  # baked old runs (read-only on Cloud)

def _pick_writable_base() -> Path:
    """
    Choose a writable base directory:
      1) M7_DATA_DIR (env/secret) if provided
      2) /mount/data/viral-script-model (Streamlit Cloud persistent volume)
      3) OS temp dir
      4) _runtime under app root (last resort)
    """
    import tempfile as _tf
    candidates = []
    env_dir = os.getenv("M7_DATA_DIR")
    if env_dir:
        candidates.append(Path(env_dir))
    candidates.append(Path("/mount/data/viral-script-model"))
    candidates.append(Path(_tf.gettempdir()) / "viral-script-model")
    candidates.append(APP_ROOT / "_runtime")
    ...
    for c in candidates:
        try:
            ...
            continue
    return APP_ROOT  # fallback (may be read-only)

DATA_BASE   = _pick_writable_base()
OUTPUT_DIR  = str(DATA_BASE / "outputs")
HISTORY_DIR = str(DATA_BASE / "outputs" / "_history")

# Create writable dirs only
Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
Path(HISTORY_DIR).mkdir(parents=True, exist_ok=True)

# Repo (read-only) dirs remain as-is
SCRIPTS_DIR = str(REPO_SCRIPTS_DIR)
PROMPTS_DIR = str(REPO_PROMPTS_DIR)
...
def render_app_title():
    ...
    .rec-meta{color:#6b7280; font-size:12.5px; margin-bottom:.4rem;}
    .rec-row{display:flex; align-items:center; justify-content:space-between; gap:12px;}
    .chip{display:inline-block; padding:2px 8px; font-size:11px; border:1px solid #d1d5db; border-radius:999px; margin-left:6px;}
    </style>
    """, unsafe_allow_html=True)
...
def _save_history_snapshot(
    ...
    try:
        ...
        st.warning(f"Could not save history snapshot to {HISTORY_DIR}. Reason: {e}")

def _load_all_history() -> List[dict]:
    """
    Load all saved runs from:
      1) Writable history dir (HISTORY_DIR) → source: data
      2) Read-only repo history dir (REPO_HISTORY_DIR) → source: repo
    Normalize created_at. Deduplicate by (run_id, created_at).
    """
    out: List[dict] = []
    seen = set()
    files: List[Tuple[str,str]] = []

    for d in [HISTORY_DIR, str(REPO_HISTORY_DIR)]:
        if d and os.path.isdir(d):
            tag = "data" if os.path.abspath(d) == os.path.abspath(HISTORY_DIR) else "repo"
            for fp in glob.glob(os.path.join(d, "*.json")):
                files.append((fp, tag))

    for fp, src_tag in sorted(files):
        try:
            with open(fp, "r", encoding="utf-8") as f:
                j = json.load(f)
        except Exception:
        ...
        j.setdefault("_path", fp)
        j.setdefault("_source", src_tag)

        run_id = j.get("run_id") or os.path.basename(fp).split("__")[-1].split(".")[0]
        created_at = j.get("created_at") or ""
        key = (run_id, created_at)
        if key in seen:
            continue
        seen.add(key)
...
def _render_recents_centerpane():
    ...
    for rec in recs:
        ...
        overall = rec.get("overall_rating", "")
        src = rec.get("_source", "")
        src_chip = f'<span class="chip">source: {src}</span>' if src else ""
        with st.container():
            st.markdown(f"""
            <div class="rec-card">
              <div class="rec-row">
                <div>
                  <div class="rec-title">{title}</div>
                  <div class="rec-meta">{created_h} {src_chip}</div>
                  <div><strong>Overall:</strong> {overall if overall != "" else "—"}/10</div>
                </div>
                <div>
            """, unsafe_allow_html=True)
...
with st.sidebar:
    st.subheader("Run Settings")
    st.caption("Prompts dir"); st.code(os.path.abspath(PROMPTS_DIR))
    st.caption("History dir"); st.code(os.path.abspath(HISTORY_DIR))  # show actual writable history path
...
def render_home():
    ...
    if st.button("🚀 Run Review", type="primary", use_container_width=True):
        ...
        # Save snapshot for Recents (into writable history dir)
        _save_history_snapshot(
            title=base_stem,
            data=data,
            script_text=script_text,
