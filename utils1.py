...
# - Optional DOCX report renderer (AOIs-first; raw extractions hidden)
# - Sanitizer for model artifacts (Decision:, Score N, leading numbers, emojis)
# - _sget: read config from env OR Streamlit secrets
# -----------------------------------------------------------------------------
...
# --- Hybrid config getter -----------------------------------------------------
# Tries env first, then Streamlit secrets (if available), else default.
try:
    import streamlit as st  # type: ignore
except Exception:  # pragma: no cover
    st = None  # type: ignore

def _sget(key: str, default: Optional[str] = None) -> Optional[str]:
    """
    Read a config value by key from:
      1) os.environ
      2) st.secrets (if Streamlit available)
      3) default
    Returns the string value or default if missing/empty.
    """
    val = os.getenv(key)
    ...
    if st is not None:
        try:
            # st.secrets behaves like a mapping in Streamlit
            if hasattr(st, "secrets") and st.secrets is not None:
                # .get handles KeyError and returns None if missing
