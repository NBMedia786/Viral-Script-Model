...
# # review_engine_multi.py — S3-only prompt loader for Runpod + robust reads
...
# # -----------------------------
# # Runpod S3 client (boto3) — S3-only mode when configured
# # -----------------------------
# import boto3
# from botocore.config import Config
...
# # --------- Env helpers (align with app_grammarly_ui) ----------
# def _get_env(key: str, default: str = "") -> str:
#     v = os.getenv(key, "")
#     if v:
#         return v.strip()
#     # If running in Streamlit with secrets, this will be no-op here; harmless
#     try:
#         import streamlit as st  # type: ignore
#         v2 = st.secrets.get(key)  # type: ignore
...
# def _s3_enabled() -> bool:
#     # We consider S3 "enabled" only when endpoint, bucket, and credentials exist
...
# def _read_s3_text(key: str) -> Optional[str]:
#     """Return object body as UTF-8 text or None (with small retries)."""
#     cli = _s3_client()
#     if not cli:
#         return None
#     tries = 4
...
# # -----------------------------------------------------------------------------#
# # Prompt loaders (S3-only when S3 is configured)
# # -----------------------------------------------------------------------------#
# def _strip_bom(s: str) -> str:
#     return s.lstrip("\ufeff") if s and s.startswith("\ufeff") else s
...
# _CONTENT_BLOCK_RE = re.compile(r"^\s*content\s*:\s*\|?\s*(.*)$", flags=re.S | re.I)
...
#       content: |
#         ...
#     extract that block; else return whole file (trimmed).
#     """
#     raw = (raw or "").strip()
#     m = _CONTENT_BLOCK_RE.search(raw)
...
# def _load_prompt_local(prompts_dir: str, n: int) -> Optional[str]:
#     """
#     Local fallback — ONLY used if S3 is not enabled.
#     """
#     for ext in ("yaml", "yml"):
#         p = os.path.join(prompts_dir, f"{n}.{ext}")
#         if os.path.isfile(p):
...
#     """
#     Unified loader:
#       - If S3 is configured: load ONLY from S3 and error if missing.
#       - If S3 is not configured: load from local files.
#     `prompts_dir_or_prefix` should be like "Scriptmodel/prompts".
#     """
#     # Normalize s3://bucket/key -> key when passed accidentally
#     prefix = prompts_dir_or_prefix
#     if prefix.startswith("s3://"):
#         parts = prefix.split("/", 3)
#         prefix = parts[3] if len(parts) > 3 else ""
...
#         if txt:
#             return txt
#         # S3 enabled but prompt missing -> hard error (S3-only mode)
#         where = f"S3({_RP_BUCKET}:{prefix})"
#         raise FileNotFoundError(f"Prompt {n} (yaml/yml) not found in {where}")

#     # S3 not enabled -> local dev fallback
#     txt = _load_prompt_local(prompts_dir_or_prefix, n)
#     if txt:
#         return txt
#     where = os.path.abspath(prompts_dir_or_prefix)
#     raise FileNotFoundError(f"Prompt {n} (yaml/yml) not found in {where}")
...
# def run_review_multi(
#     script_text: str,
#     prompts_dir: str = "Scriptmodel/prompts/",  # treated as S3 prefix when S3 is enabled
#     temperature: float = 0.0,
#     include_commentary: bool = False,  # kept for API parity; ignored
# ) -> str:
#     """
...
#     NOTE: When RUNPOD_S3_* env vars are set (and credentials provided), this loader is S3-only.
#     If S3 is not configured, it falls back to local file prompts.
#     """
#     _require_api_key()
#     llm = _make_llm(temperature)
...
#     # ---- Optional: load full signature and build style windows
#     sig_path = os.getenv("SIGNATURE_SCRIPT_PATH", "reference/signature_script.docx")
#     signature_text = _load_docx_text(sig_path)
#     max_windows = int(os.getenv("STYLE_WINDOWS_PER_CALL", "4") or "4")
#     max_chars   = int(os.getenv("STYLE_WINDOW_MAX_CHARS", "500") or "500")
