...
# Optional toggles to mirror the app hardening
_USE_SSL    = (_sget("RUNPOD_S3_USE_SSL", "true") or "true").lower() in {"1", "true", "yes"}
_VERIFY_SSL = (_sget("RUNPOD_S3_VERIFY_SSL", "true") or "true").lower() in {"1", "true", "yes"}
...
def _s3():
    ...
    if endpoint and not endpoint.startswith(("http://", "https://")):
        endpoint = "https://" + endpoint  # normalize like in the app
    ...
    _S3 = boto3.client(
        ...
        config=Config(
            signature_version="s3v4",
            s3={"addressing_style": "path"},  # path-style for gateways
            retries={"max_attempts": 10, "mode": "adaptive"},
            read_timeout=90,
            connect_timeout=15,
            tcp_keepalive=True,
...
def _jitter_delay(base: float, attempt: int, cap: float = 12.0) -> float:
    # exponential backoff with jitter
    return min((base * (2 ** attempt)) + random.random() * 0.6, cap)
...
def _s3_get_bytes(key: str, tries: int = 6) -> Optional[bytes]:
    ...
    for i in range(tries):
        try:
            ...
            # retry on network or 5xx/429
            status = None
            try:
                status = e.response.get("ResponseMetadata", {}).get("HTTPStatusCode")  # type: ignore[attr-defined]
            except Exception:
                status = None
            if isinstance(e, EndpointConnectionError) or status in (429, 500, 502, 503, 504):
                last = e
                time.sleep(_jitter_delay(0.5, i))
                continue
            return None
        except Exception as e:  # pragma: no cover
            last = e
...
def _s3_list(prefix: str) -> List[str]:
    """
    Lighter, resilient lister:
      - adds Delimiter to cut down payload
      - uses small pages (MaxKeys=200)
    ...
    def _page(tok):
        kw = {"Bucket": _RP_BUCKET, "Prefix": pfx, "Delimiter": "/", "MaxKeys": 200}
    ...
    while True:
        try:
            ...
            if not token:
                break
            attempts = 0  # reset after a good page
        except (ClientError, EndpointConnectionError, NoCredentialsError, BotoCoreError) as e:
            attempts += 1
            if attempts <= 5:
                time.sleep(_jitter_delay(0.4, attempts))
                continue
            # degrade gracefully
            break
        except Exception:
...
def _to_legacy_param_block(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    suggestion = str(raw.get("suggestion", "") or "").strip()
    suggestions_list: List[str] = []
    if not suggestion and isinstance(raw.get("suggestions"), list) and raw["suggestions"] is not None:
        suggestions_list = [str(s).strip() for s in raw["suggestions"] if str(s).strip()]
        suggestion = suggestions_list[0] if suggestions_list else ""
    elif isinstance(raw.get("suggestions"), list):
        suggestions_list = [str(s).strip() for s in raw["suggestions"] if str(s).strip()]
