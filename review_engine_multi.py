...
# Uses the shared normalizer + the hybrid secrets accessor (_sget) from utils1
from utils1 import normalize_review_payload, _sget
...
def _require_api_key() -> None:
    """
    Ensure GOOGLE_API_KEY exists. Source: os.environ OR Streamlit secrets.
    Exposes the key back to the environment for client libs.
    """
    key = _sget("GOOGLE_API_KEY")
    if not key:
        raise EnvironmentError("GOOGLE_API_KEY not found in environment or Streamlit secrets")
    os.environ["GOOGLE_API_KEY"] = key  # make sure the SDK can see it
